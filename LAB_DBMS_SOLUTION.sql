...
/*3*/
select customer.CUS_GENDER,count(customer.CUS_GENDER) from customer inner join `order` 
...
/*5*/
select * from supplier,product_details where supplier.SUPP_ID = product_details.SUPP_ID 
group by product_details.SUPP_ID having count(product_details.SUPP_ID)>1;


/*6*/
select category.CAT_NAME from category,`order`,product_details,product where `order`.PROD_ID = product_details.PROD_ID and product_details.PRO_ID = product.PRO_ID and 
product.CAT_ID = category.CAT_ID having min(`order`.ORD_AMOUNT);



/*7*/
select product.PRO_ID, product.PRO_NAME from product join product_details on product.PRO_ID =product_details.PRO_ID
